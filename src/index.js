...
function getInitialWhisperComponents() {
  return [new ActionRowBuilder().addComponents(
    new UserSelectMenuBuilder().setCustomId("menu_whisper_user").setPlaceholder("Fısıldanacak bir kullanıcı seçin...")
  )];
}
...
function getInitialWhisperEmbed() {
  return new EmbedBuilder()
    .setColor("#9B59B6")
    .setTitle("🤫 Gizli Fısıltı Odası Kur")
    .setDescription("Aşağıdaki menüyü kullanarak anonim fısıltı başlatmak istediğiniz kullanıcıyı seçin.");
}
...
client.on("ready", async () => {
  ...
  try {
    await client.application.commands.set([
      {
        name: "fısıltı",
        description: "Gizli ve anonim bir fısıltı odası başlatır."
      }
    ]);
...
client.on("messageCreate", async msg => {
  ...
  // Hedef kullanıcının thread içerisindeki yanıtlarını yakalayıp panele yansıtma paneli
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    if (interaction.commandName === "fısıltı") {
      return interaction.reply({ embeds: [getInitialWhisperEmbed()], components: getInitialWhisperComponents(), ephemeral: true });
    }
  }
  ...
  if (interaction.isUserSelectMenu()) {
    if (interaction.customId === "menu_whisper_user") {
      ...
      return interaction.reply({ 
        embeds: [new EmbedBuilder().setColor("#9B59B6").setTitle("🖼️ Fısıltı Formatı Seçin").setDescription(`<@${targetId}> kullanıcısına açılacak fısıltı odasındaki ilk mesajın biçimini seçin:`)], 
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId.startsWith("modal_wh_submit_")) {
      ...
      try {
        ...
        return interaction.reply({ content: "❌ Gizli başlık odası oluşturulurken bir hata kapıyı çaldı. Yetkilerimi kontrol edin.", ephemeral: true }); 
      }
    }
